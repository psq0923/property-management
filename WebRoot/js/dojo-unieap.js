...
(function(){
	...
	dojo.mixin(dojo, {
		...
		try{
			return !module ? this._loadUri(uri, cb,module) : this._loadUriAndCheck(uri, module, cb); // Boolean
		}catch(e){
			console.debug(e);
			return false; // Boolean
		}
	}
	/*统一接口,避免版本不一,DongYuwei updated.*/
    dojo.getJsCache = function(){
		 try{
		 	if(top.document.frames.length > 0){   
				if(top.document.frames("frmjs") && top.document.frames("frmjs").jsCache){
					dojo.getJsCache = function(){
					   return top.document.frames("frmjs").jsCache;
					} 
					return top.document.frames("frmjs").jsCache;//电力方案
				}
	      	}
			if(top.jsCache){
				dojo.getJsCache = function(){
	      	...
	      	return top.jsCache={};//key point
	      }
	}
	
	dojo.getCacheDB = function(){
	   try{
			if(top._cache_db){
				dojo.getCacheDB = function(){
					return top._cache_db;
				}
				return top._cache_db;
			}
			if(top.document.frames.length > 0){
				if(top.document.frames("frmJs") && top.document.frames("frmJs")._cache_db){
				   dojo.getCacheDB = function(){
					     return top.document.frames("frmJs")._cache_db;
				   }
				   return top.document.frames("frmJs")._cache_db;
				}
			}	
	...
	dojo._loadUri = function(/*String (URL)*/uri,/*Function?*/cb, /*String*/moduleName){
		//	summary:
		...
		if(moduleName && ((typeof moduleName) =="string")){//moduleName可能会为function
			var cache = d.getJsCache() || d.global.jsCache;
			cache[moduleName]=contents;
			//console.info(jsCache);
		}
	...
	dojo._loadUriAndCheck = function(/*String (URL)*/uri, /*String*/moduleName, /*Function?*/cb){
		...
		var ok = false;
		try{
			ok = this._loadUri(uri, cb, moduleName);
		}catch(e){
	...
	dojo._loadModule = dojo.require = function(/*String*/moduleName, /*Boolean?*/omitModuleCheck){
		...
		var module = this._loadedModules[moduleName]||d.getObject(moduleName,false);
		if(module){
			return module;
		}
		if(window.unieap && window.unieap.ClassNameMapping && window.unieap.ClassNameMapping[moduleName]){
			moduleName = unieap.ClassNameMapping[moduleName];
		}
		/////////////////////////////////增加js缓存//////////////////////////////
		...
		var cache = d.getJsCache();
		if(cache && cache[moduleName] ){
			d["eval"](cache[moduleName]);
			return (this._loadedModules[moduleName]= d.getObject(moduleName, false));
		}
...
if(typeof window != 'undefined'){
	...
	dojo._loadInit = function(e){
		...
		//node.style.cssText = "overflow:hidden;position:static;";
		if(document.body){
			//document.body.insertAdjacentElement("afterBegin",node);
			dojo.place(node,document.body,"first");
		}
...
//			});
			_handleNodeEvent("onunload", function(){
				if(_unloading){ 
				  ...
				  evtArray = null;
				  for(var i=0,l=dojo._loaders.length;i<l;i++){
				  	 dojo._loaders[i] = null;
				  }
				  dojo._loaders = null;
				  for(var i=0,l=dojo._unloaders.length;i<l;i++){
				  	 dojo._unloaders[i] = null;
				  }
				  dojo._unloaders = null;
				  var _ie_handlers = dojo._ie_listener.handlers
				  for(var i=0,l=_ie_handlers.length;i<l;i++){
				     _ie_handlers[i] = null;
				  }
				  _ie_handlers = null;
				  dojo._ie_listener = null;
				  dojo.doc.onmousedown = null;
				  dojo.global.onfocus = null;
				  dojo.doc = null;
				  dojo.global = null;
				  try{
		...
		dojo.mixin(del, {
			...
			_stealthKeyDown: function(evt){
				...
				if(unprintable||evt.ctrlKey){
					...
					//tell onkeypree evtent from onkeydown
					evt.formkeydown=true;
...
dojo.parser = new function(){
	...
	function getClassInfo(/*String*/ className){
		...
		if(!instanceClasses[className]){
			...
			var params={};
			if(proto["_attributeMap"]){
				params = dojo.mixin({id:"string","class":"string",style:"string",jsId:"string",securityId:"string"},proto["_attributeMap"]);
			}
			else{
	...
	this.parse = function(/*DomNode?*/ rootNode){
		...
		/*
	    var nodes = rootNode?rootNode.all:document.all;
		var list = [];
		var scripts = [];
		var loadedscript = {};
		var loadedwidget = {};
		for(var i = 0,node; node = nodes[i++];){
		   ...
		   if(dojoType){
		   	  if(!loadedwidget[dojoType]&&unieap.WidgetDepending[dojoType]){
		   	  	 for(var j = 0,s ;s = unieap.WidgetDepending[dojoType][j++];){
		   	  	    if(!loadedscript[s]){
		   	  	    	scripts.push(s);
		   	  	    	loadedscript[s] = true;
		   	  	    }
		   	  	 }
		   	  	 loadedwidget[dojoType] = true;
		   	  }
		   	  list.push(node);
		   }
		}
		...
		if(cache){
			...
			for(var i=0,script;script = scripts[i++];){
			    scriptContents.push(cache[script])
			}
			window.eval(scriptContents.join("\n"));
		}
...
if(!dojo._hasResource["unieap.ClassNameMapping"]){
dojo._hasResource["unieap.ClassNameMapping"] = true;

dojo.provide("unieap.ClassNameMapping");
	/**
	 * 映射不正确、不标准的类名到正确的文件（相对）路径，保证标签解析、自动加载类时不出错。
	 * 注意，要保证key和value都是String类型。
	 * @see dojo.parser
	 * */
	unieap.ClassNameMapping={
		"unieap.grid.PagingToolBar":"unieap.widget.grid._paging.PagingToolBar",
		"unieap.widget.Grid" :"unieap.widget.grid.Grid",
		"unieap.cache":"unieap.cache.cache",
		"unieap.widget.Menu":"unieap.widget.menu.Menu",
		"unieap.form.RadioGroup":"unieap.form.CheckBox",
		"unieap.form.RadioButton":"unieap.form.CheckBox",
		"unieap.widget.TitlePane" : "unieap.layout.TitlePane",
		"unieap.form.DropDownButton":"unieap.form.Button",
		 "unieap.Toolbar":"unieap.toolbar",
		"unieap.widget.GridPanel":"unieap.widget.grid.GridPanel" 
	};
	unieap.WidgetDepending={
		"unieap.form.ValidationTextBox":["unieap.form._FormWidget","unieap.form.BindingMix","dijit.Tooltip","unieap.ToolTip","unieap.form.ValidationTextBox"],
		"unieap.form.DateTextBox":["unieap.form._FormWidget","unieap.form.BindingMix","dijit.Tooltip","unieap.ToolTip","unieap.form.DateTextBox"],
		"unieap.form.ComboBox":["dojox._sql._crypto","dojox._sql.common","dojox.sql","unieap.form._FormWidget","unieap.form.BindingMix","dojo.data.util.sorter","dojo.data.util.simpleFetch","dojo.data.util.filter","dojo.data.ItemFileReadStore",
	         "unieap.data.ItemFileReadStore", "unieap.form.ComboBox"],
	    "unieap.widget.Grid":["unieap.widget.grid._data.api","dojox.grid._data.editors","unieap.widget.grid._data.editors","unieap.widget.grid._data.parser","dojox.grid._data.fields",
	         "unieap.widget.grid._data.model","dojox.grid._grid.publicEvents","dojox.grid._grid.edit","dojox.grid._grid.selection","dojox.grid._grid.focus",
	         "dojox.grid._grid.rows","dojox.grid._grid.cell","dojox.grid._grid.layout","dojox.grid._grid.views","dojox.grid._grid.drag","dojox.grid._grid.builder","dojox.grid._grid.view"
	         ,"dojox.grid._grid.rowbar","unieap.widget.grid._data.rowbar","dojox.grid._grid.scroller","dojox.grid._grid.lib","dojox.grid.VirtualGrid",
	         "unieap.widget.grid.GridExtend","unieap.widget.grid.Grid"],
	    "unieap.form.Button":["dijit.form._FormWidget","dijit.form.Button","unieap.form.Button"],
	    "unieap.form.Form":["unieap.form.Form"],
	    "unieap.layout.TitlePane":["unieap.layout.TitlePane"],
	    "unieap.form.CheckBox":["dijit.form._FormWidget","dijit.form.Button","dijit.form.CheckBox","unieap.form.CheckBox"],
	    "unieap.layout.VerticalContainer":["unieap.layout.VerticalContainer"],
	    "unieap.widget.query.Query":["dijit.layout.ContentPane","unieap.layout.ContentPane","dijit.form._FormWidget","unieap.form._FormWidget","unieap.form.BindingMix","dijit.form.Button","unieap.form.Button",
	                                 "dijit.form.CheckBox","unieap.form.CheckBox","dijit.Tooltip","unieap.ToolTip","unieap.form.DateTextBox","unieap.widget.query.Query"],
	    "unieap.grid.PagingToolBar":["unieap.widget.grid._paging.PagingToolBar"],
	    "unieap.layout.TabContainer":["dijit.layout.ContentPane","unieap.layout.ContentPane","dijit.layout.StackContainer","dijit.layout.TabContainer","unieap.layout.TabContainer"]
	};
}
